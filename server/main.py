...
engine = create_engine('sqlite:////C:\Schual\SWP\Zettelkatalog\zettelkatalog\server\catalog.db') #sqlite
...
class CatalogREST(Resource):
    ...
    def patch(self, id):
        ...
        if info is None:
            return jsonify({'message': 'object with id %d does not exist' % id})
        description_get = request.json['params']['description']
        info.description = description_get
        #lucas
        infoHistory = CatalogHistory(des_id=info.id, description=description_get, datum=date.today())
        db_session.add(infoHistory)
        db_session.add(info)
        db_session.flush()
...
#lucas
class CatalogRESTHistory(Resource):
    def get(self, id):
        infos = CatalogHistory.query.filter(CatalogHistory.des_id == id).all()
        return  jsonify(infos)
...
api.add_resource(CatalogRESTHistory, '/cat-item-history/<int:id>')
